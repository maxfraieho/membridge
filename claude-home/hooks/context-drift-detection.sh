...
if [ ! -f "$SESSION_FILE" ]; then
    echo "⚠️  WARNING: Session file not found: $CURRENT_SESSION"
    exit 0
fi
...
if [ "$FILE_AGE_DAYS" -gt 5 ]; then
    ...
    echo "⚠️  WARNING: Session age > 3 days ($FILE_AGE_DAYS days)"
...
if [ "$FILE_SIZE" -gt 1000 ]; then
    echo "⚠️  WARNING: Session file very large ($FILE_SIZE lines)"
    echo "   Compression strongly recommended"
elif [ "$FILE_SIZE" -gt 500 ]; then
    echo "ℹ️  INFO: Session file growing ($FILE_SIZE lines)"
fi
