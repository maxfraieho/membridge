...
- **API Endpoints:** The Control Plane exposes endpoints for project and agent management, and for triggering sync operations. The Agent Daemon (v0.4.0) provides endpoints for health checks (with OS/disk/capabilities reporting), project status, sync push/pull/doctor, self-update (git pull + pip install + service restart), service restart (systemd/OpenRC/SIGHUP), uninstall (stop + disable + cleanup), git clone (for multi-project management), repo listing, and system info. The agent auto-registers with BLOOM Runtime on startup and reports capabilities. The BLOOM Runtime exposes an Express API for managing LLM tasks, leases, workers, artifacts, and audit logs, along with proxying Membridge Control Plane requests.
