...
- **API Endpoints:** The Control Plane exposes endpoints for project and agent management, and for triggering sync operations. The Agent Daemon (v0.4.0) provides endpoints for health checks (with OS/disk/capabilities reporting), project status, sync push/pull/doctor, self-update (git pull + pip install + service restart), service restart (systemd/OpenRC/SIGHUP), uninstall (stop + disable + cleanup), git clone (for multi-project management), repo listing, and system info. The agent auto-registers with BLOOM Runtime on startup and reports capabilities. **Claude CLI execution** is handled via `POST /execute-task` on the agent — receives task payload, runs `claude --print -p "prompt"` with session context persistence (context_id→session file), sends heartbeats, and posts completion results back to BLOOM Runtime. Session memory is persisted in `~/.membridge/sessions/{context_id}.json`. The BLOOM Runtime exposes an Express API for managing LLM tasks, leases, workers, artifacts, and audit logs, along with proxying Membridge Control Plane requests. **Task dispatch** (`POST /api/runtime/llm-tasks/:id/dispatch`) auto-leases + sends the task to a worker for Claude CLI execution in one step.
...
- **Installation Modes:** The `install.sh` script supports various modes for agent-only, server-only, or combined installations, including migration and cleanup functionalities. The BLOOM Runtime generates install scripts via `GET /api/runtime/agent-install-script` that auto-detect systemd/OpenRC, configure sudoers for passwordless service management, set up `BLOOM_RUNTIME_URL` for auto-registration, and create `~/projects` for multi-project repos.
